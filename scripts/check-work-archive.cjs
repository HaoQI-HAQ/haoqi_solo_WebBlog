...
(async () => {
  ...
  try {
    ...
    await page.evaluate(() => localStorage.setItem('haoqi-signal-seen', '1'));
    await page.reload();
    assert.equal(await page.locator('.desktop-nav a[aria-current="page"]').textContent(), 'Home', 'Home must be marked as the active navigation item');
    await page.locator('.round-link').click();
    await page.waitForURL('**/work.html');
    assert.equal(await page.locator('.desktop-nav a[aria-current="page"]').textContent(), 'Work', 'Work must be marked as the active navigation item');
